...
async function loadPayer() {
    ...
    try {
        const secret = JSON.parse(await readFile(defaultPath, 'utf8'));
        return Keypair.fromSecretKey(new Uint8Array(secret));
    }
    catch {
        try {
            const secret = JSON.parse(await readFile(FALLBACK_PAYER_FILE, 'utf8'));
            return Keypair.fromSecretKey(new Uint8Array(secret));
        }
        catch {
...
async function saveScriptAccount(pubkey, transactionId) {
    const { writeFile } = await import('fs/promises');
...
async function run() {
    ...
    const preferred = ["init_counter", "get_value"];
    ...
    for (const functionName of targets) {
        const functionDef = program.getFunction(functionName);
        const params = functionDef?.parameters || [];
        const accountArgs = getAccountOverrides(functionName);
        const dataArgs = {};
        ...
        const computeUnits = txDetails?.meta?.computeUnitsConsumed ?? parseComputeUnitsFromLogs(txDetails?.meta?.logMessages);
